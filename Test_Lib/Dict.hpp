...
//{
//    int prime = _primes[ 0 ];
//    _nodes.reserve( prime );                            // 确保空间足够
//    memset( _nodes.data(), 0, _nodes.byteSize() );      // _size 后面的单元要用来存 已分配但未使用的指针，所以也要初始化
//    _buckets.resize( prime, false );                    // 将桶的空间留出来
...
//{
//    int prime;
//    // todo: move to reserve and call reserve or other static prime finder func like Utils::nextPrime( currIdx, capacity )
//    for( int i = 0; i < _countof( _primes) ; ++i )      // 遍历质数表 看看有没有刚好大于等于传入 capacity 的值，有就用之
//    {
