...
namespace Sqlite
{
    ...
    void Connection::beginTranslation()
    {
    ...
    void Connection::endTranslation()
    {
    ...
    Query::Query( Connection * conn, std::string const & sql, int n )
        : _conn( conn )
        , _stmt( nullptr )
        , _vi( 0 )
    {
    ...
    bool Query::execute( RowReaderType rr )
    {
        ...
        if( _vc != _vi ) return false;
        _vi = 0;
        int ok = sqlite3_step( _stmt );
        if( !( ok == SQLITE_OK || ok == SQLITE_DONE || ok == SQLITE_ROW ) ) return false;
        if( rr )
        {
            while( ok == SQLITE_ROW )
            {
    ...
    {
        if( s ) return add( std::string( s ) );
        return sqlite3_bind_null( _stmt, ++_vi ) == SQLITE_OK;
    }

    bool Query::add( char const * buf, size_t len )
    {
